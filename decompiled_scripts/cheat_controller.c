...
void main() // Position - 0x0 (0)
{
	...
	while (true)
	{
		...
		if (NETWORK::NETWORK_IS_GAME_IN_PROGRESS() && _NETWORK_IS_PLAYER_VALID(PLAYER::PLAYER_ID(), true, true) && !Global_1836575 && Global_33832 == 0)
			func_113();
...
void func_4(Hash hParam0) // Position - 0x1F6 (502)
{
	...
	if (STREAMING::HAS_MODEL_LOADED(hParam0))
	{
		if (PLAYER::IS_PLAYER_PLAYING(PLAYER::PLAYER_ID()))
		{
			...
			{
				if (!ENTITY::IS_ENTITY_DEAD(veLocal_85, false))
				{
					if (PED::IS_PED_IN_VEHICLE(PLAYER::PLAYER_PED_ID(), veLocal_85, false))
					{
						entityCoords = { ENTITY::GET_ENTITY_COORDS(veLocal_85, true) };
						entityHeading = ENTITY::GET_ENTITY_HEADING(veLocal_85);
						ENTITY::SET_ENTITY_COORDS(PLAYER::PLAYER_PED_ID(), entityCoords, entityCoords.f_1, entityCoords.f_2 + 4f, true, false, false, true);
						flag = true;
					}
				}
			
				VEHICLE::DELETE_VEHICLE(&veLocal_85);
			}
			...
			{
				veLocal_85 = VEHICLE::CREATE_VEHICLE(hParam0, entityCoords, entityHeading, false, true, false);
				VEHICLE::SET_VEHICLE_ON_GROUND_PROPERLY(veLocal_85, 1084227584);
...
void func_5(int iParam0) // Position - 0x339 (825)
{
	...
	{
		MISC::SET_BIT(&Global_34083, iParam0);
		Global_34086 = 1;
		func_6();
	}
...
void func_9() // Position - 0x3F6 (1014)
{
	...
	if (ENTITY::DOES_ENTITY_EXIST(PLAYER::PLAYER_PED_ID()))
	{
		if (_GET_CHARACTER_MODEL(Global_114990.f_2370.f_539.f_4321) != ENTITY::GET_ENTITY_MODEL(PLAYER::PLAYER_PED_ID()))
		{
			...
			if (func_10(character) && !func_15(14) || Global_113936)
			{
...
{
	if (IS_BIT_SET(Global_34081, iParam0))
		return true;
...
void func_31(int iParam0, BOOL bParam1) // Position - 0xBD2 (3026)
{
	if (bParam1)
	{
		MISC::SET_BIT(&Global_34082, iParam0);
		func_5(iParam0);
	}
	else
	{
		MISC::CLEAR_BIT(&Global_34082, iParam0);
	}
	...
	switch (iLocal_79)
	{
		...
		case 5:
			...
			if (SCRIPT::GET_NUMBER_OF_THREADS_RUNNING_THE_SCRIPT_WITH_THIS_HASH(joaat("appinternet")) == 0)
				func_40(iLocal_90 - iLocal_92, "CHEAT_INV" /*INVINCIBILITY*/, 0, 0, 1000, 0, 2, 0, 6, 0, 0, 0, 0, 0, 0, 0, 0, -1);
...
{
	if (IS_BIT_SET(Global_34082, iParam0))
		return true;
			...
			if (PED::IS_PED_IN_ANY_VEHICLE(PLAYER::PLAYER_PED_ID(), false))
			{
				...
				if (!ENTITY::IS_ENTITY_DEAD(vehiclePedIsIn, false))
				{
					if (VEHICLE::GET_PED_IN_VEHICLE_SEAT(vehiclePedIsIn, -1, false) == PLAYER::PLAYER_PED_ID())
					{
						if (veLocal_86 == 0)
						{
							veLocal_86 = vehiclePedIsIn;
						
							if (!ENTITY::IS_ENTITY_DEAD(veLocal_86, false))
								if (func_82(1))
									VEHICLE::SET_VEHICLE_REDUCE_GRIP(veLocal_86, true);
								else
									veLocal_86 = 0;
						}
						else if (veLocal_86 != vehiclePedIsIn)
						{
							if (!ENTITY::IS_ENTITY_DEAD(veLocal_86, false))
								VEHICLE::SET_VEHICLE_REDUCE_GRIP(veLocal_86, false);
...
{
	if (IS_BIT_SET(Global_34084, iParam0))
	{
		if (iParam1 != 0)
		{
			if (iParam1 == Global_34085)
			{
				MISC::CLEAR_BIT(&Global_34084, iParam0);
				bLocal_110 = true;
				return true;
			}
			else
...
void func_108() // Position - 0x3C40 (15424)
{
	...
	for (i = 0; i < 10; i = i + 1)
	{
		if (uLocal_97[i] != -1)
		{
			HUD::THEFEED_REMOVE_ITEM(uLocal_97[i]);
			uLocal_97[i] = -1;
		}
	}
